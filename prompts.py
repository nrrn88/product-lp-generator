...
- **重要: 見出し（h2, h3, h4等）、テーブル、リストには絶対に `style="..."` を付与しないでください（グローバルCSSと競合するため）。**
  - **NG例**: `<h2 style="color:red;">見出し</h2>`, `<table style="border:1px;">...`
  - **OK例**: `<h2>見出し</h2>`, `<table>...`
  - ※背景色等の装飾が必要な場合は、見出しやテーブルを囲む `<div>` に `style` を適用してください。
- **しかし、読み手が退屈しないよう、以下の装飾は積極的にインラインスタイル（style="..."）で実装してください:**
  - 重要なポイントやメリットを囲む「背景色付きボックス」（例: `background-color: #f4f8ff; border-radius: 8px; padding: 16px;`）。
  - テキストの強調色（赤文字、オレンジマーカー線など）。
  - **絵文字は一切使用しないでください（信頼性を損なうため）。**
  - **「地味で硬い」印象にならないよう、ボックス装飾や配色で視覚的なメリハリをつけてください。**
- 構成: 
